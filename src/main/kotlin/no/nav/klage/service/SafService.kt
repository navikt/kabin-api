...
class SafService(
    ...
    private fun mapTema(temaer: List<Tema>): List<no.nav.klage.clients.saf.graphql.Tema> =
        temaer.map { tema ->
            no.nav.klage.clients.saf.graphql.Tema
                .valueOf(tema.name)
        }

    fun getJournalpostAsSaksbehandler(journalpostId: String): Journalpost? = safGraphQlClient.getJournalpostAsSaksbehandler(journalpostId)
}
